...
# set up matplotlib
is_ipython = 'inline' in matplotlib.get_backend() # hecks if the string 'inline' is present in the name of the current backend.
if is_ipython: # checks if code is running in an IPython environment (like Jupyter Notebook)
    from IPython import display #  for controlling the display of output, including the display of Matplotlib plots
...
def select_action(state):
    ...
    steps_done += 1
    if sample > eps_threshold: # (1 - epsilon) chance of choosing greedy action i.e. what our policy network says the best action is
        with torch.no_grad():
...
def optimize_model():
    if len(memory) < BATCH_SIZE:
        return
    transitions = memory.sample(BATCH_SIZE) # transitions is an array/list of transitions e.g. [Transition(s2, a2, s3, r3), Transition(s35, a35, s36, r36), ....]. And here, s2 is like the 417 elt tensor representing wordle state, a2 is like a single-valued tensor representing guess word, etc.  reward is also a 1 elt tensor

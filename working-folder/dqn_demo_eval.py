...
env = WordleEnv500()
eval = True # Set to true if you want to evaluate the model
load_path = './dqn_wordle_data_500.pth' # Path to the model you want to load
...
if eval:
    policy_net.load_state_dict(torch.load(load_path)) # Load data in my saved file
...
for i_episode in range(num_episodes):
    ...
    for t in count():  # This python loop loops forever until we reach the end of the episode (at which point we break outta the loop). t takes on values 0, 1, 2 3...
        ...
        if done:
            ...
            if not eval and (i_episode % 50 == 0 or i_episode == (num_episodes - 1)):
                data = plot_durations()
...
episode_durations_wins_df.to_csv("episode_durations_wins.csv", index=False)
